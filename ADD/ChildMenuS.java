...
public class ChildMenuS extends JFrame implements ActionListener {
        ...
        save.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {// if the button is clicked
                    for(int i = 0; i<UPCList.size(); i++){
                        Child birth = new Child(SKUList.get(i).getText(),ParentMenu.fields[0].getText()+", "+SizeList.get(i).getText(),
                            UPCList.get(i).getText(), SizeList.get(i).getText(),
                            "",Integer.parseInt(QList.get(i).getText()), parent);
                    }
                    ChildMenuS.super.dispose();
                }          
            });
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        Sizel.setBounds(210,10,100,25);
        row.add(Sizel);

        JTextField size = new JTextField();
        size.setBounds(260,10,100,25);
        row.add(size);
        SizeList.add(size);
